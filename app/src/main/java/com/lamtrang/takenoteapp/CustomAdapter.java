...
public class CustomAdapter extends CursorAdapter {
    ...
    public void bindView(View view, Context context, Cursor cursor) {
        ...
        String content = cursor.getString(
                cursor.getColumnIndexOrThrow(TakeNoteTable.NoteEntry.COLUMN_CONTENT));
        boolean isImportant = cursor.getInt(
                cursor.getColumnIndexOrThrow(TakeNoteTable.NoteEntry.COLUMN_IS_IMPORTANT)) > 0;
        String strDate = cursor.getString(
                cursor.getColumnIndexOrThrow(TakeNoteTable.NoteEntry.COLUMN_CREATED_DATE));
